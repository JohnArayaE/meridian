...
`set_paused`, admin key rotation, and keeper-secret rotation — the three
things `apps/docs/overview/trust-model.md` (Trust Model, #726, not yet
merged as of this writing) and
[Mainnet Deployment](./mainnet-deployment.md#rollback-plan) both point here
for instead of duplicating.
...
- The vault's admin events (`paused`, `transfer`, `accept`, `adapter`,
  ...
  `packages/shared/src/constants.ts`) — e.g.
...
- `getRpcAdminHistory` (`packages/stellar-sdk-helpers/src/admin-history.ts`)
  ...
  script against `packages/stellar-sdk-helpers`, not curling anything —
  reasonable during an active incident if a block explorer's UI isn't
  giving you what you need (raw event data, a specific ledger range), but
  don't reach for it as step one when the explorer link above already
...
a depositor's funds — and it does **not** block `begin_migration`,
...
want to stop new deposit exposure while you investigate — even if you
haven't yet confirmed whether the situation is actually dangerous.
...
4. If a malicious `transfer_admin` is discovered _before_ its matching
   ...
   pending nomination directly — but the legitimate admin can immediately
   call `transfer_admin` again with the legitimate successor, which
   overwrites the malicious nomination (`transfer_admin`'s doc comment in
   `packages/contracts/vault/src/lib.rs` confirms it "overwrites any prior,
   not-yet-accepted nomination"). This only works if the legitimate admin
   key can still sign — if the same compromised key is what an attacker
   used to submit the malicious nomination in the first place, overwriting
   it doesn't help, since the attacker can just resubmit.

## Keeper-secret rotation

Three keepers, three different secrets, three very different stakes if one
leaks — treat them accordingly rather than as one undifferentiated "rotate
the keeper key" task:

- **`MERIDIAN_KEEPER_SECRET_KEY`** (accrue keeper). `accrue()` is
  permissionless — this key only needs to be a funded Stellar account, not
  anything vault-privileged. Rotation is low-stakes: generate a new funded
...
- **`MERIDIAN_MIGRATION_KEEPER_SECRET_KEY`** (migration keeper). Per
  ...
  **this key must be the vault's actual admin address** — `migrate_adapter`
  is admin-gated, so this secret _is_ the admin key in practice.
...
- **`MERIDIAN_ALERT_WEBHOOK_URL`** (alert keeper). Lowest stakes of the
  ...
  send fake-looking alerts to your Slack/Discord or see real ones — an
  annoyance and a possible confusion vector during a real incident, not a
  fund-safety issue. Rotate by generating a new incoming-webhook URL from
